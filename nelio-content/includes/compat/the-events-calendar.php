...
function include_events( $events ) {
	...
	$found_events = array_map(
		function ( $data ) {
			...
			return array(
				'id'              => 'tribe_event-' . $id,
				'date'            => 'draft' === $data->post_status ? null : $data->post_date_gmt,
				'start'           => 'draft' === $data->post_status ? null : get_post_meta( $id, '_EventStartDateUTC', true ) . ' +00:00',
				'end'             => 'draft' === $data->post_status ? null : get_post_meta( $id, '_EventEndDateUTC', true ) . ' +00:00',
