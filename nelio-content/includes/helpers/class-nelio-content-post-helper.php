...
class Nelio_Content_Post_Helper {
	...
	private static $networks = array( 'bluesky', 'facebook', 'instagram', 'linkedin', 'mastodon', 'medium', 'pinterest', 'twitter', 'tumblr', 'telegram', 'tiktok', 'threads', 'gmb', 'reddit' );
	...
	public function get_permalink_query_args( $post_id ) {
		...
		return empty( $args ) || ! is_array( $args ) ? array() : $args;
	}//end get_permalink_query_args()
	 ...
	 *
	 * @param int   $post_id Post ID.
	 * @param array $args    List of query arg pairs (name and value).
	 *
	 * @since 3.3.0
	 */
	...
	}//end update_permalink_query_args()
	...
	private function get_the_content( $post ) {
		 ...
		 * @since 3.6.0
		 */
		do_action( 'before_the_nelio_content' );
		 ...
		 * @since 2.3.1
		 */
		$content = apply_filters( 'the_nelio_content', $content, $post->ID );
